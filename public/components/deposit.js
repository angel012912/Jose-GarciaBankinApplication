    ...
    <>
      {
        user ? 
          ...
          body={show ? 
            ...
            <SuccessMessage setShow={setShow} setStatus={setStatus} message="Deposit again"/>}
        /> :
...
function DepositForm(props){
  ...
  function handle(){
    const url = `/account/transaction/${props.user.email}/deposit/${amount}`;
      (async ()=>{
        var res = await fetch(url);
        var data = await res.json();
      })().then(()=>{
  ...
  return(<>
    ...
    <button type="submit" 
      ...
      onClick={handle}
      disabled={disableAmount}>Deposit</button>
